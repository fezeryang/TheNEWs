...
### 2026/01/23 - v5.4.0 (本分支新增)

> **新增：GEO 借势营销分析系统**

- **GEO 模式**：全新的营销机会挖掘模式，将热点流量转化为商业机会
- **4 步转换漏斗**：
  - 实体识别：自动识别品牌、产品、行业等营销实体
  - 营销价值判断：从 6 个维度评估新闻的营销潜力（0-100 评分）
  - 逻辑桥接：设计 Hook→Bridge→CTA 转换话术
  - 竞品对比：自动生成竞品分析框架
- **配置增强**：
  - 新增 `ai_analysis.mode` 配置项（generic | geo）
  - 新增 `ai_analysis.geo_focus` 配置项（聚焦特定领域）
  - 新增 GEO 专用 prompt 文件 `ai_analysis_geo_prompt.txt`
- **渲染优化**：所有推送渠道（飞书、钉钉、企微、邮件等）支持 GEO 模式显示
- **文档完善**：新增 `GEO_ANALYSIS_GUIDE.md` 完整使用指南

> 💡 **使用场景**：营销团队、品牌方、咨询公司寻找借势机会  
> 💡 **详细指南**：[GEO_ANALYSIS_GUIDE.md](GEO_ANALYSIS_GUIDE.md)
...
TrendRadar 提供两种 AI 分析模式，满足不同场景需求：

#### **通用模式（Generic）** - 理解热点趋势

使用 AI 大模型对热点新闻进行深度分析，输出 5 大核心板块：

1. **核心热点态势** - 提炼共性叙事，挖掘底层逻辑
2. **舆论风向争议** - 绘制情绪光谱，识别观点冲突
3. **异动与弱信号** - 捕捉轨迹突变，预警潜在趋势
4. **RSS 深度洞察** - 专家视角补充，修正大众认知
5. **研判策略建议** - 预测推演，给出行动指南

> 💡 通用模式适合：投资者、研究员、自媒体人、普通用户

#### **GEO 模式（Growth through Event Opportunities）** - 借势营销系统 🔥

**核心目标**：将公共领域的热点新闻（流量）转化为企业服务（产品）的关注度

GEO 模式提供 4 步营销转换漏斗：

| 步骤 | 功能 | 输出内容 |
|-----|------|---------|
| 1️⃣ **实体识别** | 自动识别品牌、产品、行业等营销实体 | 热度评分（0-100）、提及次数、行业类别 |
| 2️⃣ **营销价值判断** | 从 6 个维度评估新闻的营销潜力 | 争议性、新品发布、排名变化、话题热度、时效性、行业影响 |
| 3️⃣ **逻辑桥接** | 设计从"吃瓜"到"客户咨询"的转换话术 | Hook（勾子）→ Bridge（桥梁）→ CTA（行动召唤） |
| 4️⃣ **竞品对比** | 自动识别竞品并生成对比框架 | 竞品列表、对比维度、战略洞察 |

**示例输出**：
```
Hook: "小米汽车又出新招——SU7降价促销"
Bridge: "但你有没有想过，为什么它要降价？背后的战略逻辑是什么？"
CTA: "我们来做一份《小米汽车 vs 竞品策略对比报告》，看看谁的策略更高明"
转化率预估: 35-45%
```

> 💡 GEO 模式适合：营销团队、品牌方、咨询公司、业务拓展人员
>
> 💡 详细使用指南见 [GEO_ANALYSIS_GUIDE.md](GEO_ANALYSIS_GUIDE.md)
...
ai_analysis:
  mode: "generic"  # 通用模式
  # mode: "geo"    # GEO 借势营销模式
  prompt_file: "ai_analysis_prompt.txt"       # 通用模式 prompt
  # prompt_file: "ai_analysis_geo_prompt.txt" # GEO 模式 prompt
```
