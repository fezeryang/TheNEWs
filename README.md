...
如果你是**品牌方、市场人员或自媒体运营者**，可以开启 **GEO 模式**，让 AI 从热点新闻中自动识别营销机会。

**GEO 模式 vs 通用模式对比**：

| 功能 | 通用模式 | GEO 模式 |
|-----|---------|---------|
| 分析目标 | 趋势洞察、情感分析 | 营销机会识别 |
| 实体识别 | 不支持 | ✅ 自动识别品牌/产品 |
| 营销价值判断 | 无 | ✅ 6维评分（新品、争议等） |
| 话术转换 | 无 | ✅ 从"看新闻"→"做分析" |
| 竞品对比 | 无 | ✅ 自动生成对比框架 |

**开启方法**（二选一）：

1. **通过环境变量**（推荐，无需修改配置文件）：
   ```bash
...
**详细配置指南**：[docs/GEO_AI_CONFIGURATION.md](docs/GEO_AI_CONFIGURATION.md)

#### 进阶玩法：自定义 AI "人设"

觉得 AI 说话太官方？你可以修改它的提示词，让它变成你喜欢的风格（比如"毒舌评论员"、"资深投资顾问"）。

- **修改文件**：`config/ai_analysis_prompt.txt`（通用模式）或 `config/ai_geo_prompt.txt`（GEO 模式）
